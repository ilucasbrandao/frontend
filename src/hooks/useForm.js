 ...
 */
const useForm = (initialState, initialData = null, validationFn = () => ({})) => {
    const [formData, setFormData] = useState(initialData || initialState);
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState(null);
    const [validationErrors, setValidationErrors] = useState({}); // NOVO: Erros de validação local

    // Sincroniza quando initialData muda (modo edição)
    useEffect(() => {
        if (initialData) {
            const cleanData = { ...initialData };
            
            // Limpeza e formatação de data para input[type="date"]
...
        }
        setValidationErrors({}); // Limpa erros ao mudar dados iniciais
        
        // Nota: 'initialState' é omitido das dependências,
        // assumindo que é uma constante ou um objeto memoizado na função chamadora.
...
            const newState = { ...prev, [name]: val };
            ...
            if (newErrors[name]) {
                setValidationErrors(prevErrors => ({ ...prevErrors, [name]: newErrors[name] }));
            } else {
...
        
        // Validação ao definir valor programaticamente
...
    const resetForm = useCallback((data = initialState) => {
        setFormData(data);
        setValidationErrors({});
    }, [initialState]);
...
            setError(submitError.response?.data?.message || 'Erro ao processar sua solicitação.');
...
        error, 
        setError,
        validationErrors, // NOVO: Erros de validação
        handleSubmit, // NOVO: Função para lidar com o submit
    };
};
